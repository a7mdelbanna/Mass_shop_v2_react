...
import {
  createItemOfferDetailsByCompany,
  createItemOfferDetailsByCategory,
} from "@/services/offer-service";
import { useTranslation } from "react-i18next";

interface ItemOfferBulkAddDialogProps {
  open: boolean;
  onClose: () => void;
  storeId: string;
  offerId: number;
...
const ItemOfferBulkAddDialog: React.FC<ItemOfferBulkAddDialogProps> = ({
  open,
  onClose,
  storeId,
  onSuccess,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!offerId || (!discountValue && !discountPercent) || isBigUnit === null)
      return;
    setLoading(true);
    try {
      const payload = {
        ...
        itemOfferId: Number(offerId),
        isBigUnit: isBigUnit, // boolean directly
      };

      if (mode === "company") {
        await createItemOfferDetailsByCompany(storeId, {
          companyId: Number(companyId),
          ...payload,
        });
      } else {
        await createItemOfferDetailsByCategory(storeId, {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{t("addItemsToOffer")}</DialogTitle>
          <DialogDescription>{t("bulkAddItemsToOfferDesc")}</DialogDescription>
        </DialogHeader>
...
export default ItemOfferBulkAddDialog;
